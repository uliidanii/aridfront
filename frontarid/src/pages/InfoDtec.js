...
const InfoDtec = () => {
  ...
  useEffect(() => {
    async function fetchImage() {
      try {
        const response = await api.get(`http://54.210.56.185:8080/api/tecnicos/${user.id}/imagen`, {
  ...
  const uploadImage = async (userId, file) => {
    ...
    try {
      await api.post(`http://54.210.56.185:8080/api/tecnicos/${userId}/imagen`, formData, {
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await api.put(`http://54.210.56.185:8080/api/tecnicos/${user.id}`, formData);
