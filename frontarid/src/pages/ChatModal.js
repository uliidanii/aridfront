...
const ChatModal = ({ show, handleClose, tecnicoId, incidenciaId,onNewMessage,estadoIncidencia, }) => {
  ...
  const SERVER_BASE_URL = 'http://3.83.99.31:8080';
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await axios.get(
        `http://3.83.99.31:8080/api/conversations/${docenteId}/${tecnicoId}/${incidenciaId}`
      );
  ...
  useEffect(() => {
    ...
    const socket = new SockJS('http://3.83.99.31:8080/ws');
