...
const ChatModal = ({ show, handleClose, tecnicoId, incidenciaId,onNewMessage,estadoIncidencia, }) => {
  ...
  const SERVER_BASE_URL = 'http://44.204.79.85:8080';
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await axios.get(
        `http://44.204.79.85:8080/api/conversations/${docenteId}/${tecnicoId}/${incidenciaId}`
      );
  ...
  useEffect(() => {
    ...
    const socket = new SockJS('http://44.204.79.85:8080/ws');
