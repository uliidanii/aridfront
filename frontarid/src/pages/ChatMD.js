...
const ChatMD = ({ show, handleClose, docenteId, incidenciaId,estadoIncidencia }) => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await axios.get(
        `http://3.83.99.31:8080/api/tecnico/conversations/${tecnicoId}/${docenteId}/${incidenciaId}`
      );
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const response = await axios.post("http://3.83.99.31:8080/upload-file", formData);
