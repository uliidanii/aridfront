...
const ChatMD = ({ show, handleClose, docenteId, incidenciaId,estadoIncidencia }) => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await axios.get(
        `http://44.204.79.85:8080/api/tecnico/conversations/${tecnicoId}/${docenteId}/${incidenciaId}`
      );
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const response = await axios.post("http://44.204.79.85:8080/upload-file", formData);
